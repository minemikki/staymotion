...
import { ingestGeneration, buildFinal } from './production.js';
import { stitchToBlob } from './reel.js';
...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Hent en Higgsfield-generering (video-URL) rett inn i en bestilling: serveren laster ned filen, lagrer master + web-preview (H.264, faststart) + poster, og legger den under PRODUKSJON i admin som et shot/take Michael kan forhåndsvise og godkjenne. Bruk denne for HVER generering — ikke gi Michael en URL han må laste ned selv.',
    inputSchema: {
      type: 'object',
      properties: {
        ref: { type: 'string', description: 'Ordre-referanse, f.eks. SM-XXXXXX' },
        url: { type: 'string', description: 'Direkte URL til Higgsfield-videoen' },
        shotId: { type: 'string', description: 'Shot-id, f.eks. shot_01 (default shot_01)' },
        shotLabel: { type: 'string', description: 'Lesbar tittel på shotet, f.eks. "Eksteriør"' },
        take: { type: 'string', description: 'Take-bokstav, f.eks. a, b, c (default a)' },
        model: { type: 'string', description: 'Higgsfield-modell brukt, f.eks. seedance_2_0' },
      },
      required: ['ref', 'url'],
      additionalProperties: false,
    },
  ...
  {
    name: 'deliver_final',
    description: 'Legg den ferdige, sammensatte videoen til en bestilling som FINAL (QC). Serveren henter fila, lagrer master + web-preview + poster under final/, og den dukker opp i «Final levering» i admin — Michael trykker GODKJENN LEVERING for at kunden skal se den. Bruk denne når hele reelen er ferdig klippet.',
    inputSchema: {
      type: 'object',
      properties: {
        ref: { type: 'string' },
        url: { type: 'string', description: 'Direkte URL til den ferdige videoen (f.eks. /api/reel-resultatet)' },
        name: { type: 'string', description: 'Valgfritt filnavn' },
      },
      required: ['ref', 'url'],
      additionalProperties: false,
    },
  ...
  {
    name: 'build_reel',
    description: 'Sett sammen flere klipp (video-URLer) til ÉN ferdig reel (1080x1920, 24fps, faststart) på serveren, og legg den rett inn i bestillingen som FINAL (QC) klar for Michaels godkjenning. Bruk denne når alle klippene er laget og skal bli én video — da slipper Michael å laste ned/opp noe.',
    inputSchema: {
      type: 'object',
      properties: {
        ref: { type: 'string', description: 'Ordre-referanse' },
        clips: { type: 'array', items: { type: 'string' }, description: 'Klipp-URLer i rekkefølge (utvendig først, så rom for rom)' },
      },
      required: ['ref', 'clips'],
      additionalProperties: false,
    },
...
async function runTool(name, args, host) {
  ...
  if (name === 'get_order') {
    ...
    return {
      ...
      produksjon: (o.production && o.production.shots || []).map((sh) => ({
        shot: sh.id, tittel: sh.label, status: sh.status,
        takes: (sh.takes || []).map((t) => ({ take: t.take, godkjent: !!t.approved, modell: t.model, previewUrl: t.preview && t.preview.url })),
      })),
      samtale: []
  ...
  if (name === 'ingest_generation') {
    const found = await findOrder(args.ref);
    if (!found) return { error: 'Fant ingen bestilling med ref ' + args.ref };
    if (!args.url) return { error: 'Mangler url til genereringen' };
    try {
      const r = await ingestGeneration({
        orderId: found.id, url: args.url,
        shotId: args.shotId, shotLabel: args.shotLabel, take: args.take, model: args.model,
      });
      return { ok: true, ref: found.id, shot: r.shot, take: r.take, previewUrl: r.preview, posterUrl: r.poster };
    } catch (e) {
      return { error: 'Kunne ikke hente inn genereringen: ' + e.message };
    }
  }
  if (name === 'deliver_final') {
    const found = await findOrder(args.ref);
    if (!found) return { error: 'Fant ingen bestilling med ref ' + args.ref };
    if (!args.url) return { error: 'Mangler url til videoen' };
    try {
      const r = await buildFinal({ orderId: found.id, url: args.url, name: args.name });
      return { ok: true, ref: found.id, versjon: r.version, status: 'qc_venter_godkjenning', previewUrl: r.previewUrl };
    } catch (e) {
      return { error: 'Kunne ikke lagre final: ' + e.message };
    }
  }
  if (name === 'build_reel') {
    const found = await findOrder(args.ref);
    if (!found) return { error: 'Fant ingen bestilling med ref ' + args.ref };
    const clips = Array.isArray(args.clips) ? args.clips.filter(Boolean) : [];
    if (!clips.length) return { error: 'Ingen klipp oppgitt' };
    try {
      const saved = await stitchToBlob(clips, {});
      const r = await buildFinal({ orderId: found.id, url: saved.url, name: 'staymotion_' + found.id + '_reel.mp4' });
      return { ok: true, ref: found.id, klipp: saved.klipp, versjon: r.version, status: 'qc_venter_godkjenning', previewUrl: r.previewUrl };
    } catch (e) {
      return { error: 'Kunne ikke bygge reel: ' + e.message };
    }
  }
