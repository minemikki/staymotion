// StayMotion MCP server (Model Context Protocol over Streamable HTTP).
// Exposes the order workflow as tools so a Claude session can, on its own:
//   list new paid orders → pull the customer's photo URLs → set status →
//   reply to the customer. Dependency-free, stateless JSON-RPC 2.0.
//
// Auth: send the bearer token in the Authorization header
//   Authorization: Bearer <MCP_TOKEN>   (or ?key=<MCP_TOKEN>)
// The token lives ONLY in Vercel env (MCP_TOKEN) and in the connector
// settings — never in code or chat.

import { listOrders, getOrderView, setStatus, addReply } from '../lib/orders.js';
  ...
  if (name === 'reply_to_customer') {
    ...
    if (email) {
      try {
        ...
        try {
          const token = signOrder({ orderId: found.id, email, exp: Date.now() + 1000 * 60 * 60 * 24 * 90 });
          portalUrl = `${origin}/ordre.html?ref=${encodeURIComponent(found.id)}&t=${encodeURIComponent(token)}`;
        } catch (e) {}
...
export default async function handler(req, res) {
  ...
  const given = auth.replace(/^Bearer\s+/i, '') || (req.query && req.query.key) || '';
  if (given !== token) return res.status(401).json({ error: 'Ikke autorisert' });

  // GET is only used by clients that want a server-initiated SSE stream. This
  // server is stateless request/response, so we don't hold one open.
  ...
  for (const m of msgs) {
    ...
    try {
      if (m.method === 'initialize') {
        ...
        out.push(rpcResult(id, {}));
      } else if (m.method === 'notifications/initialized' || (m.method && m.method.startsWith('notifications/'))) {
