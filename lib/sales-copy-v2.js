...
function profile(industry=''){
  ...
  return {kind:'generisk',label:'bedriften',proof:'tilbudet og det som skiller dere ut',action:'kontakt',value:'gjøre det enklere for de riktige kundene å forstå tilbudet og ta kontakt'};
}
...
function verifiedPositive(research={},p){
  ...
  if(!human) return '';
  if(/prosjekt|referanse/i.test(human)) return p.kind==='salong'?'Dere har innhold som kan brukes bedre til å vise frem resultater og stil.':'Dere har prosjekt-/referanseinnhold som kan brukes enda tydeligere som salgsbevis.';
  if(/kontaktvei|kontakt/i.test(human)) return 'Dere har allerede en tydelig vei videre for folk som vil komme i kontakt.';
  if(/omtale|anmeld/i.test(human)) return 'Dere har tillitsbevis som kan brukes enda tydeligere i nettsiden.';
  return '';
}
...
function opportunityText(research={},p,location=''){
  ...
  if(p.kind==='salong'){
    if(hasProof) parts.push('resultater, behandlinger og selve salongopplevelsen kan løftes tydeligere frem');
    if(hasCta) parts.push('veien fra førsteinntrykk til booking kan bli enklere');
  ...
  if(!parts.length) return 'Jeg tror nettsiden kan presentere tilbudet tydeligere, gi et sterkere førsteinntrykk og gjøre veien til '+p.action+' enklere.';
  let text='Det jeg tror kan løftes mest, er hvordan nettsiden presenterer '+p.proof+'. Jeg ser blant annet at '+parts[0];
  if(parts[1]) text+=', samtidig som '+parts[1];
  text+='.';
  if(parts[2]) text+=' I tillegg tror jeg '+parts[2]+'.';
  return text;
}
...
export function buildSalesDraft(lead={},research={}){
  ...
  const visual='Et mer moderne og ryddig visuelt uttrykk kan også gi et sterkere førsteinntrykk og gjøre '+p.action+' enklere å få øye på.';
  ...
  return {subject,body,followup1:{subject:'Re: '+subject,body:follow1},followup2:{subject:'Re: '+subject,body:follow2},copyVersion:4,generatedAt:Date.now()};
}
