...
module.exports = `
:root{
  --ground:#0E0B09;--raised:#17120F;--raised2:#1F1815;
  --ember:#D9622E;--ember-l:#E89A62;--ember-d:#A8431A;
  --bone:#F0E9DD;--ash:#8A8078;--ash-d:#5E564F;
  --line:#2A2320;--line-l:#3A312C;
...
/* ---------- ordmerke ---------- */
.mark{font-family:var(--sans);font-weight:600;font-size:.98rem;letter-spacing:.52em;text-transform:lowercase;color:var(--bone);padding-left:.52em}
...
/* ---------- topplinje ---------- */
.top{position:absolute;z-index:20;inset:0 0 auto;display:flex;align-items:center;justify-content:space-between;gap:1.5rem;padding:1.9rem var(--pad)}
.top nav{display:flex;gap:clamp(1.5rem,2.6vw,2.8rem)}
.top nav a{font-size:.76rem;font-weight:500;letter-spacing:.16em;text-transform:uppercase;color:var(--bone);opacity:.72;transition:opacity .2s}
.top nav a:hover{opacity:1}
.top-r{display:flex;align-items:center;gap:1.4rem}
.lang{display:flex;box-shadow:inset 0 0 0 1px rgba(240,233,221,.28)}
.lang button{min-width:40px;min-height:32px;font-size:.66rem;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:var(--bone);opacity:.6;transition:.2s}
.lang button[aria-pressed=true]{background:var(--bone);color:var(--ground);opacity:1}
.lang button:hover{opacity:1}
.burger{display:none;position:relative;width:44px;height:44px}
.burger span{position:absolute;left:11px;width:22px;height:1.5px;background:var(--bone);transition:transform .25s ease,opacity .25s ease}
...
.mnav{display:none;position:fixed;z-index:19;inset:0;flex-direction:column;justify-content:center;gap:1.5rem;padding:6rem var(--pad) 3rem;background:var(--ground)}
.mnav a{font-family:var(--disp);font-size:2.4rem;letter-spacing:-.02em;color:var(--bone)}
.mnav[hidden]{display:none}

/* ---------- hero ---------- */
.hero{position:relative;min-height:88svh;display:flex;flex-direction:column;justify-content:flex-end;overflow:hidden;background:var(--ground);isolation:isolate}
.hero__img{position:absolute;inset:0;z-index:-2}
.hero__img img{width:100%;height:100%;object-fit:cover;object-position:56% 46%;filter:brightness(.46) saturate(.88) contrast(1.06)}
.hero::after{content:"";position:absolute;inset:0;z-index:-1;pointer-events:none;
  background:linear-gradient(180deg,rgba(14,11,9,.86) 0%,rgba(14,11,9,.24) 32%,rgba(14,11,9,.62) 72%,var(--ground) 100%),
             radial-gradient(120% 78% at 76% 66%,rgba(217,98,46,.28),transparent 62%)}
.hero__in{width:100%;padding-block:clamp(3rem,7vw,6rem) clamp(2rem,4vw,3.2rem)}
.hero h1{max-width:11ch;margin-top:1.6rem}
.hero__sub{margin-top:1.8rem;max-width:46ch;font-size:clamp(1rem,1.35vw,1.16rem);line-height:1.6;color:rgba(240,233,221,.8)}
.hero__cta{display:flex;flex-wrap:wrap;align-items:center;gap:1.2rem 2rem;margin-top:2.6rem}
.hero__note{margin-top:1.5rem;font-size:.82rem;color:var(--ash)}
.hero__foot{display:flex;flex-wrap:wrap;gap:.6rem 2.4rem;margin-top:clamp(2.4rem,5vw,4rem);padding-top:1.5rem;border-top:1px solid rgba(240,233,221,.16);font-size:.78rem;letter-spacing:.06em;color:rgba(240,233,221,.62)}

/* live åpent-status */
.status{display:inline-flex;align-items:center;gap:.6rem;font-size:.7rem;font-weight:600;letter-spacing:.2em;text-transform:uppercase;color:var(--ash)}
.status i{width:6px;height:6px;flex:none;border-radius:50%;background:var(--ash-d)}
.status.on{color:var(--ember-l)}
...
/* ---------- 03 ilden ---------- */
.ild{position:relative;background:var(--ground);border-top:1px solid var(--line);overflow:hidden;isolation:isolate}
.ild::before{content:"";position:absolute;z-index:-1;left:50%;bottom:-46%;width:min(1500px,150%);aspect-ratio:1;transform:translateX(-50%);pointer-events:none;
  background:radial-gradient(circle,rgba(217,98,46,.30) 0%,rgba(168,67,26,.14) 36%,transparent 66%)}
.ild__facts{display:grid;grid-template-columns:repeat(3,1fr);border-top:1px solid var(--line);margin-top:clamp(2rem,4vw,3rem)}
...
.ild__f b{display:block;font-family:var(--disp);font-style:italic;font-weight:400;font-size:1.6rem;color:var(--ember-l)}
...
.rom__s b{display:block;font-family:var(--disp);font-weight:400;font-size:clamp(2.2rem,4vw,3.2rem);line-height:1;color:var(--ember-l)}
...
/* ---------- footer ---------- */
.foot{background:var(--ground);border-top:1px solid var(--line);padding-block:clamp(2.6rem,5vw,4rem)}
.foot__in{display:flex;flex-wrap:wrap;justify-content:space-between;align-items:flex-end;gap:1.6rem 3rem}
.foot__note{max-width:44ch;font-size:.74rem;line-height:1.6;color:var(--ash-d)}

/* ---------- fast bookinglinje på mobil ---------- */
.bookbar{position:fixed;z-index:18;inset:auto 0 0;display:none;padding:.7rem var(--pad) calc(.7rem + env(safe-area-inset-bottom));background:var(--raised2);border-top:1px solid var(--line-l)}
...
@media (max-width:720px){
  ...
  .mnav{display:flex}
  .mark{font-size:.84rem;letter-spacing:.42em}
