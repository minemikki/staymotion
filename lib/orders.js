...
// First response window (hours) — stored on the order as `deadline` and shown
// to the customer as "Lansering senest". Web projects are scheduled per
// package in lib/packages.js (`deliver`); this is only the fallback date the
// order carries from the moment it is created.
const DEFAULT_HOURS = 48;

export function deadlineFor(pkg, express, created) {
  const h = express ? Math.round(DEFAULT_HOURS / 2) : DEFAULT_HOURS;
...
// finished deliverables (files we uploaded under delivery/).
export async function getOrderView(id) {
  ...
  for (const b of blobs) {
    ...
    else if (rest.indexOf('/') < 0) photos.push({ url: b.url, name: rest, size: b.size, uploadedAt: b.uploadedAt ? new Date(b.uploadedAt).getTime() : null });
    // else: any other subfolder — not customer photos
  }
  order.photos = photos;
...
export async function countPhotos(id) {
  try {
    const { blobs } = await list({ prefix: PREFIX + id + '/', limit: 1000 });
    const head = PREFIX + id + '/';
    return blobs.filter((b) => { const rest = b.pathname.slice(head.length); return !b.pathname.endsWith('/order.json') && rest.indexOf('/') < 0; }).length;
  } catch (e) { return 0; }
}
