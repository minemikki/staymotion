...
// Probe a file's frame rate by parsing ffmpeg -i stderr ("NN fps"/"NN tbr").
function probeFps(file) {
  return new Promise((resolve) => {
    ...
    p.on('close', () => {
      const m = err.match(/,\s*([0-9.]+)\s*fps/) || err.match(/,\s*([0-9.]+)\s*tbr/);
      const v = m ? Math.round(parseFloat(m[1])) : null;
      resolve(v && v >= 12 && v <= 60 ? v : null);
    });
  });
}
...
export async function stitchClips(clips, opts = {}) {
  ...
  try {
    ...
    // Match the sources' own frame rate so uniform clips are never resampled
    // (resampling e.g. 30->24 drops frames and causes judder). Fall back to 30.
    let fps = parseInt(opts.fps, 10);
    if (!fps) { fps = await probeFps(files[0]) || 30; }
    ...
    args.push('-filter_complex', filter, '-map', '[out]',
      ...
      '-movflags', '+faststart', '-r', String(fps), '-an', '-y', out);
