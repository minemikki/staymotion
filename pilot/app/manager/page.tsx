...
function Manager({ session }: { session: Session }) {
  ...
  async function act(kind: 'ack' | 'resolve' | 'assign', inc: Incident) {
    ...
    try {
      ...
      toast(kind === 'resolve' ? 'Saken er løst. Statusen er oppdatert.' : kind === 'ack' ? 'Registrert som sett av deg.' : 'Oppfølging flyttet.');
      await load(db);
  ...
  return (
    <div className="wrapW rise">
      ...
      <p className="lead">{welcome ? 'Bedriften er satt opp. Første vakt har rutinene sine, og alt som meldes inn lander her.' : 'Her er det som trenger deg. Resten finner du under oppfølging.'}</p>
      ...
      <div className="dark calm" data-testid="calm">
        <div>
          <div className="eyebrow">Prioritert for deg</div>
      ...
      <div className="ops-metrics" aria-label="Driftsoversikt">
        <div><span>Trenger deg</span><strong>{loading ? '—' : needs.length}</strong><small>Sortert etter hastegrad</small></div>
        <div><span>Under oppfølging</span><strong>{loading ? '—' : watching.length}</strong><small>Registrert som sett</small></div>
        <div><span>Løst siste døgn</span><strong>{loading ? '—' : resolvedToday.length}</strong><small>Ferdigbehandlede saker</small></div>
      </div>
      ...
      <div className="sect">
        ...
        {loading ? <div className="empty" role="status">Henter saker …</div> : needs.length === 0 ? <div className="empty"><b>Ingenting venter på deg.</b>Nye saker fra ansatte vises her.</div> : (
          <div className="need" data-testid="needs">
            {needs.map((i) => (
              <div className={'need-i' + (i.severity === 'critical' ? ' critical' : '')} key={i.id} data-testid="need-item" aria-busy={busy === i.id}>
                ...
                <div>
                  ...
                  <div className="meta">
                    ...
                    <span className={'pill ' + (i.dueAt && Date.parse(i.dueAt) <= now ? 'warn' : '')}>{deadlineText(i, now)}</span>
                    {i.status === 'needs_attention' && <span className="pill warn">Krever oppfølging</span>}
      ...
      <div className="sect auto">
        <div className="card">
          <div className="eyebrow">Registrert aktivitet</div>
