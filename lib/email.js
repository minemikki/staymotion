      ...
      <tr><td align="center" style="padding:40px 16px">
        <table role="presentation" width="580" cellpadding="0" cellspacing="0" border="0" style="width:580px;max-width:100%">
          ...
          <tr><td align="center" style="${P}padding:26px 12px 0;font-size:12px;line-height:1.7;color:#71808A">
            ...
            Webdesign for norske bedrifter. Stavanger og Rogaland.
          </td></tr>
        </table>
      </td></tr>
    </table>
...
// Returns { ok, error } so callers can tell the difference between "sent" and
// "silently dropped". It still never throws — a failed receipt must not break
// a payment or an intake — but the caller can now record what happened, which
// is the only way a missing confirmation ever becomes visible.
export async function sendEmail({ to, subject, html, attachments }) {
  ...
  if (!key) {
    ...
    return { ok: false, error: 'RESEND_API_KEY er ikke satt' };
  }
  ...
  try {
    ...
    if (!r.ok) {
      ...
      return { ok: false, error: 'Resend svarte ' + r.status + ': ' + String(body).slice(0, 300) };
    }
    return { ok: true };
  } catch (e) {
