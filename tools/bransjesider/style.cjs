...
/* ---- stemningsbildet ----
   Ikke en full-bredde stripe med en pille liggende oppå. Bildet holdes inne i
   samme spaltebredde som teksten, i en ramme med samme radius og kantlinje som
   resten av siden, og bildeteksten står under — der en bildetekst hører hjemme.
   Da leser seksjonen som en del av siden i stedet for et banner limt inn. */
.lp-shot{margin:0;padding:0 var(--gutter) clamp(56px,7vw,86px)}
.lp-shot .ramme{max-width:var(--max);margin:0 auto;position:relative;border-radius:18px;overflow:hidden;
  border:1px solid var(--line);background:var(--pearl2);
  box-shadow:0 36px 70px -44px rgba(10,20,60,.42), 0 2px 6px -3px rgba(10,20,60,.10)}
.lp-shot img{display:block;width:100%;height:clamp(220px,30vw,380px);object-fit:cover}
/* Tynn lysning langs overkanten, så rammen får dybde i stedet for å ligge flatt. */
.lp-shot .ramme::after{content:"";position:absolute;inset:0;border-radius:inherit;pointer-events:none;
  box-shadow:inset 0 1px 0 rgba(255,255,255,.22)}
.lp-shot figcaption{max-width:var(--max);margin:12px auto 0;font-size:12.5px;line-height:1.5;color:var(--faint)}
@media(max-width:620px){
  .lp-shot .ramme{border-radius:14px}
  .lp-shot img{height:clamp(190px,52vw,260px)}
}
