...
// Patch any subset of the project fields. If this patch is the first time the
// project moves past 'strategi' (i.e. work actually starts), the customer
// gets a short "we're in it now" email — otherwise the only progress signals
// they get are the review and launch emails below, which can leave several
// silent days right after payment.
export async function updateProject(id, patch, host) {
  const order = await loadOrder(id);
  if (!order) throw new Error('Fant ikke prosjektet');
  const p = order.project && typeof order.project === 'object' ? order.project : {};
  const wasStrategi = !p.stage || p.stage === 'strategi';
  let notifyStart = false;
  if (patch.stage !== undefined) {
    ...
    if (wasStrategi && patch.stage !== 'strategi' && !p.workStartedNotifiedAt) notifyStart = true;
    p.stage = patch.stage;
  ...
  if (notifyStart) p.workStartedNotifiedAt = Date.now();
  ...
  if (notifyStart) {
    ...
    if (email) {
      try {
        ...
        await sendEmail({
          to: email,
          subject: 'Vi er i gang med nettsiden din',
          html: renderEmail({
            kicker: 'Fremdrift',
            heading: first ? ('Hei ' + first + ' — vi har startet.') : 'Vi har startet på prosjektet ditt.',
            html: emailP('Design og bygging av nettsiden din er i gang. Du kan følge fremdriften når som helst i prosjektportalen.')
              + emailP('Du hører fra oss igjen så snart et utkast er klart for gjennomgang.'),
            ctaText: 'Åpne prosjektportalen', ctaUrl: portal,
            refLabel: 'Prosjekt', refValue: String(id).toUpperCase(),
          }),
        });
      } catch (e) { console.error('[updateProject] start email', e.message); }
    }
  }
  return order;
}
