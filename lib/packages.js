...
//
// Web projects are sold intake → proposal → full payment at start. No package
// is sold as a split/deposit payment — `depositPct` is 100 everywhere and only
// exists so `resolvePackage()` has one formula for every package. Videre is
// billed monthly and never through one-time checkout.
...
export const PACKAGES = {
  ...
  launch: {
    ...
    fromKr: 7900, depositPct: 100, deliver: 'ca. 7–14 dager',
    legacy: true, supersededBy: 'landing',
  },
  growth: {
    id: 'growth', name: 'Vekst', tagline: 'Nettside med flere sider',
    fromKr: 12900, depositPct: 100, deliver: 'ca. 2–3 uker',
...
// The oldest reel orders predate the current record shape: they have no `pkg`
// field at all, and carry the package inside the reference instead
// ("sm-enkelt-1788551672981-rff0od"). Checking `pkg` alone let those through.
// So match on every signal such an order actually carries.
const REEL_REF = new RegExp('^sm-(' + REEL_PACKAGE_IDS.join('|') + ')-', 'i');
const REEL_LABEL = /cinematisk|\breel\b/i;

export function isReelOrder(order) {
  if (!order) return false;
  if (order.project) return false;                       // moved into the web workflow
  if (REEL_PACKAGE_IDS.indexOf(order.pkg) !== -1) return true;
  if (REEL_REF.test(order.id || '')) return true;
  if (REEL_LABEL.test(order.pakke || '')) return true;
  return false;
}
